...
module.exports = metalsmith(__dirname) // __dirname defined by node.js: name of current working
  ...
  // build the entrypoint for IE8 application specific JavaScript
  .use(rollup({
    input: 'src/javascripts/application-ie8-only.js',
    output: {
      legacy: true,
      format: 'iife',
      file: 'javascripts/application-ie8-only.js'
    },
    plugins: [
  ...
  .use(concat({
    files: [
      ...
      'javascripts/application-ie8-only.js'
    ],
    output: 'javascripts/application-ie8.js'
  }))
